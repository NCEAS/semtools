...
\FOR { (each $Row\langle A1, A2, \cdots, An\rangle \in Dataset$)}
    ...
    \FOR{(each $ObsTypeId \in ObsType2MeasIdx.keys$)}
...
\begin{algorithmic}
	...
	\FOR {(each $m = \langle MeasTypeId, ResAttribute, Cond, Val\rangle \in A.Map$)}
		...
		\STATE {\bf if}{(($m.ResAttribute != Row.Ai.Attrname$) OR ($Row.Ai$ does not satisfy $m.Cond$))}
		  {\bf continue};
		...
		\STATE {\bf else} $MeasVal$ = $Row.Ai.Val$;
...
\begin{algorithmic}
	...
	\IF{ ($IsObsDistinct$==true) }
        %/*From the current observation key index get the id of the observation instance*/
    ...
    \STATE {\bf if} {($ObsId$ is a new one)}
