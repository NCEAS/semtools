...
\from{HP}{More description comes later.}
...
9).\>\>{\bf for} each basic query $OM_{b}$ in $ContextQuery$\\
...
4).\>{\bf for} (each basic context $OM_{b}\rightarrow COM_{b}$ in $OMQ$)\\
5).\>\>$added$ := false;\\
6).\>\>{\bf for} (each set $oneContextChain \in contextChainDNF$)\\
7).\>\>\>{\bf if} (($OM_{b} \in oneContextChain$) or $OM_{b} \in oneContextChain$))\\
8).\>\>\>\> $oneContextChain$ := $oneContextChain \cup OM_{b} \cup COM_{b}$\\
9).\>\>\>\> $added$ :=true;\\
10).\>\>\>\> {\bf break;}\\
11).\>\>{\bf if}($added$==false)\\
...
\from{HP}{Output: record id or key attribute values.}
