...
\address[smb]{UC Davis Genome Center \& Dept. of Computer Science,
  Gonzaga University}
...
approach is to allow scientists to use common and familiar
...
environment can improve the speed and understanding of ontology
construction and avoid locating, downloading, and interpreting
additional software packages and add-ons (e.g., CMAP-COE
\cite{hayes05:_collab_knowl_captur_in_ontol}).  Many spreadsheet
applications also contain easy-to-use features that support scientists
with the structuring process (e.g., cut/copy/paste, search/replace,
sort, split-window, annotation, track-changes, freeze panes, and so
on).
...
``Earth Realm'' ontology \cite{raskin05}
\begin{small}
\begin{tabbing}
